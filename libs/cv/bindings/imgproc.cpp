...
void CalcHist(struct Mats mats, IntVector chans, Mat mask, Mat hist,
              IntVector sz, FloatVector rng, bool acc) {
...
void CalcBackProject(struct Mats mats, IntVector chans, Mat hist,
                     Mat backProject, FloatVector rng, bool uniform) {
  std::vector<cv::Mat> images;
...
struct RotatedRect FitEllipse(PointVector pts) {
  ...
  Rect r = {bRect.boundingRect().x, bRect.boundingRect().y,
            bRect.boundingRect().width, bRect.boundingRect().height};
...
void ConvexHull(PointVector points, Mat hull, bool clockwise,
                bool returnPoints) {
...
void DilateWithParams(Mat src, Mat dst, Mat kernel, Point anchor,
                      int iterations, int borderType, Scalar borderValue) {
  cv::Point pt1(anchor.x, anchor.y);
  cv::Scalar c = cv::Scalar(borderValue.val1, borderValue.val2,
                            borderValue.val3, borderValue.val4);
...
void ErodeWithParams(Mat src, Mat dst, Mat kernel, Point anchor, int iterations,
                     int borderType) {
  cv::Point pt1(anchor.x, anchor.y);
...
void ErodeWithParamsAndBorderValue(Mat src, Mat dst, Mat kernel, Point anchor,
...
struct Moment Moments(Mat src, bool binaryImage) {
  ...
  Moment mom = {m.m00,  m.m10,  m.m01,  m.m20,  m.m11,  m.m02,  m.m30,  m.m21,
...
double ContourArea(PointVector pts) { return cv::contourArea(*pts); }
...
struct RotatedRect MinAreaRect(PointVector pts) {
  ...
  Rect r = {bRect.x, bRect.y, bRect.width, bRect.height};
  Point centrpt = {int(lroundf(cvrect.center.x)),
                   int(lroundf(cvrect.center.y))};
  Size szsz = {int(lroundf(cvrect.size.width)),
               int(lroundf(cvrect.size.height))};
...
struct RotatedRect2f MinAreaRect2f(PointVector pts) {
  ...
  RotatedRect2f retrect = {(Contour2f){rpts, 4}, r, centrpt, szsz,
                           cvrect.angle};
  return retrect;
}
...
int ConnectedComponents(Mat src, Mat labels, int connectivity, int ltype,
                        int ccltype) {
...
int ConnectedComponentsWithStats(Mat src, Mat labels, Mat stats, Mat centroids,
  ...
  return cv::connectedComponentsWithStats(*src, *labels, *stats, *centroids,
...
void MorphologyExWithParams(Mat src, Mat dst, int op, Mat kernel, Point pt,
                            int iterations, int borderType) {
...
void CornerSubPix(Mat img, Mat corners, Size winSize, Size zeroZone,
...
void GoodFeaturesToTrack(Mat img, Mat corners, int maxCorners, double quality,
                         double minDist) {
  cv::goodFeaturesToTrack(*img, *corners, maxCorners, quality, minDist);
}

void GrabCut(Mat img, Mat mask, Rect r, Mat bgdModel, Mat fgdModel,
...
void HoughCirclesWithParams(Mat src, Mat circles, int method, double dp,
                            double minDist, double param1, double param2,
                            int minRadius, int maxRadius) {
  cv::HoughCircles(*src, *circles, method, dp, minDist, param1, param2,
                   minRadius, maxRadius);
}
...
void HoughLinesPWithParams(Mat src, Mat lines, double rho, double theta,
                           int threshold, double minLineLength,
                           double maxLineGap) {
  cv::HoughLinesP(*src, *lines, rho, theta, threshold, minLineLength,
                  maxLineGap);
}
...
void HoughLinesPointSet(Mat points, Mat lines, int linesMax, int threshold,
  ...
  cv::HoughLinesPointSet(*points, *lines, linesMax, threshold, minRho, maxRho,
                         rhoStep, minTheta, maxTheta, thetaStep);
}
...
void AdaptiveThreshold(Mat src, Mat dst, double maxValue, int adaptiveMethod,
...
void CircleWithParams(Mat img, Point center, int radius, Scalar color,
...
void Ellipse(Mat img, Point center, Point axes, double angle, double startAngle,
             double endAngle, Scalar color, int thickness) {
  cv::Point p1(center.x, center.y);
...
void EllipseWithParams(Mat img, Point center, Point axes, double angle,
                       double startAngle, double endAngle, Scalar color,
                       int thickness, int lineType, int shift) {
  cv::Point p1(center.x, center.y);
  cv::Point p2(axes.x, axes.y);
  cv::Scalar c = cv::Scalar(color.val1, color.val2, color.val3, color.val4);

  cv::ellipse(*img, p1, p2, angle, startAngle, endAngle, c, thickness, lineType,
              shift);
}
...
void Rectangle(Mat img, Rect r, Scalar color, int thickness) {
  ...
  cv::rectangle(*img, cv::Point(r.x, r.y),
                cv::Point(r.x + r.width, r.y + r.height), c, thickness,
                cv::LINE_AA);
}

void RectangleWithParams(Mat img, Rect r, Scalar color, int thickness,
...
void FillPolyWithParams(Mat img, PointsVector pts, Scalar color, int lineType,
...
void Polylines(Mat img, PointsVector pts, bool isClosed, Scalar color,
               int thickness) {
  cv::Scalar c = cv::Scalar(color.val1, color.val2, color.val3, color.val4);
...
struct Size GetTextSize(const char *text, int fontFace, double fontScale,
                        int thickness) {
  return GetTextSizeWithBaseline(text, fontFace, fontScale, thickness, NULL);
}

struct Size GetTextSizeWithBaseline(const char *text, int fontFace,
...
void PutText(Mat img, const char *text, Point org, int fontFace,
...
void PutTextWithParams(Mat img, const char *text, Point org, int fontFace,
                       double fontScale, Scalar color, int thickness,
...
void WarpAffineWithParams(Mat src, Mat dst, Mat rot_mat, Size dsize, int flags,
...
void WarpPerspectiveWithParams(Mat src, Mat dst, Mat rot_mat, Size dsize,
...
Mat FindHomography(Mat src, Mat dst, int method, double ransacReprojThreshold,
                   Mat mask, const int maxIters, const double confidence) {
  return new cv::Mat(cv::findHomography(
      *src, *dst, method, ransacReprojThreshold, *mask, maxIters, confidence));
}

void DrawContours(Mat src, PointsVector contours, int contourIdx, Scalar color,
                  int thickness) {
  cv::Scalar c = cv::Scalar(color.val1, color.val2, color.val3, color.val4);
  cv::drawContours(*src, *contours, contourIdx, c, thickness);
}

void DrawContoursWithParams(Mat src, PointsVector contours, int contourIdx,
  ...
  cv::drawContours(*src, *contours, contourIdx, c, thickness, lineType,
                   vecHierarchy, maxLevel, offsetPt);
}
...
void Filter2D(Mat src, Mat dst, int ddepth, Mat kernel, Point anchor,
              double delta, int borderType) {
...
void SepFilter2D(Mat src, Mat dst, int ddepth, Mat kernelX, Mat kernelY,
  ...
  cv::Point anchorPt(anchor.x, anchor.y);
  cv::sepFilter2D(*src, *dst, ddepth, *kernelX, *kernelY, anchorPt, delta,
                  borderType);
}
...
void FitLine(PointVector pts, Mat line, int distType, double param, double reps,
             double aeps) {
...
double MatchShapes(PointVector contour1, PointVector contour2, int method,
                   double parameter) {
