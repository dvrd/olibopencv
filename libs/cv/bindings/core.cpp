...
void Mat_Inv(Mat m) { m->inv(); }

Mat Mat_Col(Mat m, int c) { return new cv::Mat(m->col(c)); }

Mat Mat_Row(Mat m, int r) { return new cv::Mat(m->row(r)); }
...
void Mat_ConvertToWithParams(Mat m, Mat dst, int type, float alpha,
                             float beta) {
...
struct ByteArray Mat_ToBytes(Mat m) {
  return toByteArray(reinterpret_cast<const char *>(m->data),
                     m->total() * m->elemSize());
}

struct ByteArray Mat_DataPtr(Mat m) {
  return ByteArray{reinterpret_cast<char *>(m->data),
...
Mat Mat_MultiplyMatrix(Mat x, Mat y) { return new cv::Mat((*x) * (*y)); }

Mat Mat_T(Mat x) { return new cv::Mat(x->t()); }
...
void Mat_AddWeighted(Mat src1, double alpha, Mat src2, double beta,
...
void Mat_BatchDistance(Mat src1, Mat src2, Mat dist, int dtype, Mat nidx,
...
void Mat_CalcCovarMatrix(Mat samples, Mat covar, Mat mean, int flags,
                         int ctype) {
  cv::calcCovarMatrix(*samples, *covar, *mean, flags, ctype);
}

void Mat_CartToPolar(Mat x, Mat y, Mat magnitude, Mat angle,
                     bool angleInDegrees) {
...
void Mat_CopyMakeBorder(Mat src, Mat dst, int top, int bottom, int left,
...
void Mat_PCACompute(Mat src, Mat mean, Mat eigenvectors, Mat eigenvalues,
                    int maxComponents) {
...
double PSNR(Mat src1, Mat src2) { return cv::PSNR(*src1, *src2); }
...
void Mat_Gemm(Mat src1, Mat src2, double alpha, Mat src3, double beta, Mat dst,
              int flags) {
...
double KMeans(Mat data, int k, Mat bestLabels, TermCriteria criteria,
...
double KMeansPoints(PointVector points, int k, Mat bestLabels,
...
void Mat_MinMaxIdx(Mat m, double *minVal, double *maxVal, int *minIdx,
                   int *maxIdx) {
  cv::minMaxIdx(*m, minVal, maxVal, minIdx, maxIdx);
}

void Mat_MinMaxLoc(Mat m, double *minVal, double *maxVal, Point *minLoc,
                   Point *maxLoc) {
  cv::Point cMinLoc;
  cv::Point cMaxLoc;
...
void Mat_MinMaxLocWithMask(Mat m, double *minVal, double *maxVal, Point *minLoc,
...
void Mat_MixChannels(struct Mats src, struct Mats dst,
                     struct IntVector fromTo) {
...
void Mat_MultiplyWithParams(Mat src1, Mat src2, Mat dst, double scale,
                            int dtype) {
...
void Mat_SetIdentity(Mat src, double scalar) { cv::setIdentity(*src, scalar); }
...
void Mat_PolarToCart(Mat magnitude, Mat degree, Mat x, Mat y,
                     bool angleInDegrees) {
...
int PointVector_Size(PointVector p) { return p->size(); }
...
int PointsVector_Size(PointsVector ps) { return ps->size(); }
...
Point2fVector Point2fVector_New() { return new std::vector<cv::Point2f>; }
...
int Point2fVector_Size(Point2fVector pfv) { return pfv->size(); }
...
void IntVector_Close(struct IntVector ivec) { delete[] ivec.val; }

RNG TheRNG() { return &cv::theRNG(); }

void SetRNGSeed(int seed) { cv::setRNGSeed(seed); }
...
double RNG_Gaussian(RNG rng, double sigma) { return rng->gaussian(sigma); }

unsigned int RNG_Next(RNG rng) { return rng->next(); }
...
void RandShuffle(Mat mat) { cv::randShuffle(*mat); }
...
void StdByteVectorInitialize(void *data) { new (data) std::vector<uchar>(); }
...
int Points2fVector_Size(Points2fVector ps) { return ps->size(); }
...
Point3fVector Point3fVector_New() { return new std::vector<cv::Point3f>; }
...
int Point3fVector_Size(Point3fVector pfv) { return pfv->size(); }
...
int Points3fVector_Size(Points3fVector ps) { return ps->size(); }
...
void SetNumThreads(int n) { cv::setNumThreads(n); }
