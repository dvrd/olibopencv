...
void Window_IMShow(const char *winname, Mat mat) {
  cv::Mat *image = static_cast<cv::Mat *>(mat);
  std::string win_name{winname};
...
int Window_WaitKey(int delay = 0) { return cv::waitKey(delay); }
...
void Trackbar_CreateWithValue(const char *winname, const char *trackname,
                              int *value, int max) {
