...
@(test)
test_mat :: proc(t: ^testing.T) {
	using testing
	mat := cv.new_mat()
	defer cv.delete_mat(mat)
	expect(t, cv.mat_empty(mat), "New Mat should be empty")
}

@(test)
test_mat_with_sizes :: proc(t: ^testing.T) {
	using testing

	// create mat with multidimensional array
	sizes := []int{100, 100, 100}
	mat := cv.new_mat_with_sizes(sizes, .CV_8U)
	defer cv.delete_mat(mat)
	expect(t, !cv.mat_empty(mat), "new_mat_with_sizes should not be empty")

	for val, i in cv.mat_size(mat) {
		expectf(t, val == sizes[i], "new_mat_with_sizes incorrect size: %v\n", cv.mat_size(mat))
	}

	expectf(
		t,
		cv.mat_rows(mat) == -1,
		"new_mat_with_sizes incorrect row count: %v\n",
		cv.mat_rows(mat),
	)
	...
	expectf(
		t,
		cv.mat_cols(mat) == -1,
		"new_mat_with_sizes incorrect col count: %v\n",
		cv.mat_cols(mat),
	)

	expectf(
		t,
		cv.mat_channels(mat) == 1,
		"new_mat_with_sizes incorrect channels count: %v\n",
		cv.mat_channels(mat),
	)

	expectf(
		t,
		cv.mat_type(mat) == .CV_8U,
		"new_mat_with_sizes incorrect type: %v\n",
		cv.mat_type(mat),
	)

	expectf(
		t,
		cv.mat_total(mat) == 100 * 100 * 100,
		"NewMatWithSizes incorrect total: %v\n",
		cv.mat_cols(mat),
	)

	// create 2x3x3 multidimensional array with 3 channels and scalar
	sizes = []int{2, 3, 3}
	s := cv.Scalar{255.0, 105.0, 180.0, 0.0}
	mat = cv.new_mat_with_sizes_from_scalar(sizes, .CV_32F, s)

	expect(t, !cv.mat_empty(mat), "new_mat_with_sizes_from_scalar should not be empty")

	for val, i in cv.mat_size(mat) {
		expectf(
			t,
			val == sizes[i],
			"new_mat_with_sizes_from_scalar incorrect size: %v\n",
			cv.mat_size(mat),
		)
	}
	...
	expectf(
		t,
		cv.mat_rows(mat) == -1,
		"new_mat_with_sizes_from_scalar incorrect row count: %v\n",
		cv.mat_rows(mat),
	)
	...
	expectf(
		t,
		cv.mat_cols(mat) == -1,
		"new_mat_with_sizes_from_scalar incorrect col count: %v\n",
		cv.mat_cols(mat),
	)
	...
	expectf(
		t,
		cv.mat_channels(mat) == 1,
		"new_mat_with_sizes_from_scalar incorrect channels count: %v\n",
		cv.mat_channels(mat),
	)

	expectf(
		t,
		cv.mat_type(mat) == .CV_32F,
		"new_mat_with_sizes_from_scalar incorrect type: %v\n",
		cv.mat_type(mat),
	)

	expectf(
		t,
		cv.mat_total(mat) == 2 * 3 * 3,
		"new_mat_with_sizes_from_scalar incorrect total: %v\n",
		cv.mat_cols(mat),
	)

	matChans := cv.split(mat)
	scalar := []f32{255.0, 105.0, 180.0}
	for c := 0; c < cv.mat_channels(mat); c += 1 {
		for x := 0; x < sizes[0]; x += 1 {
			for y := 0; y < sizes[1]; y += 1 {
				for z := 0; z < sizes[2]; z += 1 {
					s := cv.mat_get_float_at3(matChans[c], x, y, z)
					expectf(
						t,
						s == scalar[c],
						"NewMatWithSizesWithScalar incorrect scalar: %v\n",
						s,
					)
				}
			}
	...
	// create 1x2x3 multidimensional array with 3 channel and data
	sizes = []int{1, 2, 3}

	// generate byte array
	s = cv.Scalar{255.0, 123.0, 55.0, 0.0}
	mat1 := cv.new_mat_with_sizes_from_scalar(sizes, .CV_32F, s)
	defer cv.delete_mat(mat1)
	b := cv.mat_to_bytes(mat1)

	ok: bool
	mat, ok = cv.new_mat_with_sizes_from_bytes(sizes, .CV_32F, b)
	expect(t, !cv.mat_empty(mat), "new_mat_with_sizes_from_bytes should not be empty")

	mat2: cv.Mat
	mat2, ok = cv.new_mat_with_sizes_from_bytes(sizes, .CV_32F, nil)
	expect(t, !ok, "new_mat_with_sizes_from_bytes should return error with empty bytes")
	b1 := cv.mat_to_bytes(mat)
	expect(t, bytes.equal(b, b1), "new_mat_with_sizes_from_bytes byte arrays not equal")

	for val, i in cv.mat_size(mat) {
		expectf(
			t,
			val == sizes[i],
			"new_mat_with_sizes_from_bytes incorrect size: %v\n",
			cv.mat_size(mat),
		)
	}
	...
	expectf(
		t,
		cv.mat_rows(mat) == -1,
		"new_mat_with_sizes_from_bytes incorrect row count: %v\n",
		cv.mat_rows(mat),
	)
	...
	expectf(
		t,
		cv.mat_cols(mat) == -1,
		"new_mat_with_sizes_from_bytes incorrect col count: %v\n",
		cv.mat_cols(mat),
	)
	...
	expectf(
		t,
		cv.mat_channels(mat) == 1,
		"new_mat_with_sizes_from_bytes incorrect channels count: %v\n",
		cv.mat_channels(mat),
	)
	...
	expectf(
		t,
		cv.mat_type(mat) == .CV_32F,
		"new_mat_with_sizes_from_bytes incorrect type: %v\n",
		cv.mat_type(mat),
	)

	expectf(
		t,
		cv.mat_total(mat) == 1 * 2 * 3,
		"new_mat_with_sizes_from_bytes incorrect total: %v\n",
		cv.mat_cols(mat),
	)
...
@(test)
test_mat_from_bytes_with_empty_slice :: proc(t: ^testing.T) {
	using testing

	_, ok := cv.new_mat_from_bytes(600, 800, .CV_8U, []byte{})
	expect(t, !ok, "new_mat_with_sizes_from_bytes should return 'false' with empty bytes")
}

@(test)
test_mat_with_size :: proc(t: ^testing.T) {
	using testing

	mat := cv.new_mat_with_size(101, 102, .CV_8U)
	defer cv.delete_mat(mat)
	expect(t, !cv.mat_empty(mat), "new_mat_with_size should not be empty")

	expectf(
		t,
		cv.mat_rows(mat) == 101,
		"new_mat_with_size incorrect row count: %v\n",
		cv.mat_rows(mat),
	)
	...
	expectf(
		t,
		cv.mat_cols(mat) == 102,
		"new_mat_with_size incorrect col count: %v\n",
		cv.mat_cols(mat),
	)
	...
	expectf(
		t,
		cv.mat_channels(mat) == 1,
		"new_mat_with_size incorrect channels count: %v\n",
		cv.mat_channels(mat),
	)
	...
	expectf(
		t,
		cv.mat_type(mat) == .CV_8U,
		"new_mat_with_size incorrect type: %v\n",
		cv.mat_type(mat),
	)
	...
	expectf(
		t,
		cv.mat_step(mat) == 102,
		"new_mat_with_size incorrect total: %v\n",
		cv.mat_cols(mat),
	)
}

@(test)
test_mat_with_size_from_scalar :: proc(t: ^testing.T) {
	using testing

	s := cv.Scalar{255.0, 105.0, 180.0, 0.0}
	mat := cv.new_mat_with_size_from_scalar(s, 2, 3, .CV_8UC3)
	defer cv.delete_mat(mat)
	expect(t, !cv.mat_empty(mat), "new_mat_with_size_from_scalar should not be empty")

	expectf(
		t,
		cv.mat_rows(mat) == 2,
		"new_mat_with_size_from_scalar incorrect row count: %v\n",
		cv.mat_rows(mat),
	)
	...
	expectf(
		t,
		cv.mat_cols(mat) == 3,
		"new_mat_with_size_from_scalar incorrect col count: %v\n",
		cv.mat_cols(mat),
	)

	expectf(
		t,
		cv.mat_channels(mat) == 3,
		"new_mat_with_size incorrect channels count: %v\n",
		cv.mat_channels(mat),
	)
	...
	expectf(t, cv.mat_step(mat) == 9, "new_mat_with_size incorrect total: %v\n", cv.mat_cols(mat))
	...
	for c := 0; c < cv.mat_channels(mat); c += 1 {
		for x := 0; x < cv.mat_rows(mat); x += 1 {
			for y := 0; y < cv.mat_cols(mat); y += 1 {
				s := cv.mat_get_uchar(matChans[c], x, y)
				expectf(
					t,
					s == scalarByte[c],
					"NewMatWithSizesWithScalar incorrect scalar: %v\n",
...
@(test)
test_mat_from_ptr :: proc(t: ^testing.T) {
	using testing

	mat := cv.new_mat_with_size(101, 102, .CV_8U)
	defer cv.delete_mat(mat)
	pmat := cv.mat_from_ptr(mat, 11, 12, .CV_8U, 10, 10)
	defer cv.delete_mat(pmat)

	expectf(
		t,
		cv.mat_rows(pmat) == 11,
		"mat_from_ptr incorrect row count: %v\n",
		cv.mat_rows(pmat),
	)
	...
	expectf(
		t,
		cv.mat_cols(pmat) == 12,
		"mat_from_ptr incorrect col count: %v\n",
		cv.mat_cols(pmat),
	)
}
